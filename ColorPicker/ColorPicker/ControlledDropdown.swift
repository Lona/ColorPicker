...
fileprivate class StyledPopUpButtonCell: NSPopUpButtonCell {
    ...
    override func drawTitle(_ title: NSAttributedString, withFrame frame: NSRect, in controlView: NSView) -> NSRect {
        let customTitle = NSMutableAttributedString(attributedString: title)
        customTitle.addAttribute(.foregroundColor, value: textColor ?? NSColor.controlTextColor, range: .init(location: 0, length: title.length))
        return super.drawTitle(customTitle, withFrame: frame, in: controlView)
    }
}

// MARK: - ControlledDropdown

internal class ControlledDropdown: NSPopUpButton {

    override open class var cellClass: AnyClass? {
        get { return StyledPopUpButtonCell.self }
        set {}
    }
    ...
    public var textColor: NSColor? {
        get { return parameters.textColor }
        set { parameters.textColor = newValue }
    }
    ...
    private func update() {
        ...
        if let cell = cell as? StyledPopUpButtonCell {
